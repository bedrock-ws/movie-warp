...
def server(config: Config) -> Server:
    app = Server()

    @app.server_event
    async def ready(ctx: ReadyContext) -> None:
        print(f"Ready @ {color.from_name("yellow").fg:{ctx.host}:{ctx.port}}")

    @app.game_event
    async def player_message(ctx: PlayerMessageContext) -> None:
        if ctx.sender in consts.NAME:
            return
        
        message = ctx.message
        run = ctx.server.run

        if message != "moviewarp":
            return

        await run("gamemode spectator", wait=True)
        for (number, frame) in enumerate(config.video.iter_frames(fps=config.fps)):
            im = Image.fromarray(frame)
            im = im.resize(size=(config.width, config.height), resample=Resampling.BILINEAR)
            pixels = im.getdata()
            w = y = 0
            x = z = 0
            for (i, (r, g, b)) in enumerate(pixels):
                if config.direction == Direction.LATITUDINAL:
                    x = w
                else:
                    z = w
                color = closest_rgb((r, g, b), PALETTE.keys())
                block = PALETTE[color]
                await run(f"setblock ~{x} ~{config.height - y} ~{z} {block}", wait=False)
                if (w + 1) % config.width == 0:
                    w = 0
                    y += 1
                else:
                    w += 1
            await run(f"structure save moviewarp.{config.name}.frame{number} ~ ~1 ~ ~{x} ~{y} ~{z} false disk", wait=True)
